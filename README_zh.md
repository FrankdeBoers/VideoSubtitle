...
测试设备：**三星 Galaxy S24 Ultra**（Qualcomm Snapdragon 8 Gen 3 for Galaxy，4 nm；Adreno 750 GPU）。视频：**时长 1:00、720p**。下表是端到端整条管线（提取 → 识别 → 翻译 → 烧录）的总耗时：
...
总时长由识别阶段主导；提取 / 翻译 / 烧录三步在该设备上加起来不超过 1 分钟。
