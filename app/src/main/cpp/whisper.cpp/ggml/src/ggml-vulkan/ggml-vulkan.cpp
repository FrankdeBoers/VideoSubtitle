...
// VideoSubtitle patch (docs/GPU_SUPPORT_PLAN.md): switch Vulkan-Hpp to its
// dynamic dispatcher so the produced libwhisper.so doesn't have hard
// link-time references to Vulkan 1.1+ symbols. minSdk=26 still allows
// loading on Vulkan-1.0-only devices — the dispatcher's init returns null
// for unsupported entry points, and ggml_vk_instance_init() detects an
// insufficient API version and aborts cleanly so the JNI engine retries on
// CPU.
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        // VideoSubtitle patch: route through Vulkan-Hpp dynamic dispatcher
        // so we don't take a hard link-time dependency on Vulkan 1.1's
        // C symbol — see top-of-file comment.
        ...
        // VideoSubtitle patch: refine the dynamic dispatcher with the new
        // logical device so device-level entries hit fast direct pointers.
...
static void ggml_vk_print_gpu_info(size_t idx) {
    ...
    // VideoSubtitle patch: dynamic dispatcher (matches the patched call
    // in ggml_vk_get_device above).
...
static void ggml_vk_instance_init() {
    ...
    // VideoSubtitle patch: bring up the dynamic dispatcher (loader-level
    // entries only) before the first vk:: call. Subsequent passes refine
    // it with the instance and device once those are available.
    static vk::DynamicLoader vk_dl;
    auto vkGetInstanceProcAddr_fn =
        vk_dl.getProcAddress<PFN_vkGetInstanceProcAddr>("vkGetInstanceProcAddr");
    if (vkGetInstanceProcAddr_fn == nullptr) {
        std::cerr << "ggml_vulkan: failed to load vkGetInstanceProcAddr from libvulkan.so" << std::endl;
    ...
    // VideoSubtitle patch: re-init dispatcher with the new instance so all
    // instance-level entry points (vkGetPhysicalDeviceFeatures2, etc.)
    // resolve.
