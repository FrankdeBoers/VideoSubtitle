...
- **whisper.cpp ggml-vulkan backend (Phase 8 / `docs/GPU_SUPPORT_PLAN.md`).** `WHISPER_VULKAN=ON` is the default — vendored under `app/src/main/cpp/whisper.cpp/ggml/src/ggml-vulkan/` (`ggml-vulkan.cpp` + 107 `.comp` shader sources + the upstream `vulkan-shaders-gen` host tool). Build cross-compiles `vulkan-shaders-gen` for the host via `ExternalProject_Add` (host-toolchain pinned: macOS uses `MacOSX15.4.sdk` because macOS 26.4 SDK's libc++ uses `__builtin_clzg` which Apple Clang 21 doesn't have), runs it with NDK-bundled `glslc` to emit `ggml-vulkan-shaders.{hpp,cpp}`, and links the result + the Vulkan loader from the NDK sysroot into `libwhisper.so`. Vulkan headers come from **vendored** `cpp/third_party/Vulkan-Headers/` at `vulkan-sdk-1.3.290.0` — do NOT use the NDK's bundled `sources/third_party/vulkan/src/include/vulkan/vulkan.hpp`, it's at v1.3.237 and has duplicate `MemoryMapFlagBits` (etc.) enum definitions that fail to compile. `ggml-vulkan.cpp` carries three minimal `// VideoSubtitle patch` edits that flip Vulkan-Hpp into dynamic-dispatcher mode (`VULKAN_HPP_DISPATCH_LOADER_DYNAMIC=1`); without those, `libwhisper.so` would import Vulkan 1.1 symbols (`vkGetPhysicalDeviceFeatures2`, `vkEnumerateInstanceVersion`, `vkGetPhysicalDeviceProperties2`) that the API 26 NDK loader doesn't export, and `System.loadLibrary("whisper")` would `UnsatisfiedLinkError` on Android 8.0 devices. After the patch the only `UND` Vulkan imports left in the stripped lib are `vkGetInstanceProcAddr` + `vkCmdCopyBuffer` (both Vulkan 1.0). On a Vulkan-1.0-only device, `WhisperLib.gpuAvailable()` returns false and `WhisperJniEngine` falls back to CPU automatically.
- **Vulkan-Hpp throws C++ exceptions; the JNI side is C.** If `vk::Queue::submit` returns a non-success Result (most commonly `VK_ERROR_DEVICE_LOST` from a flaky GPU driver mid-graph), Vulkan-Hpp throws `vk::DeviceLostError`. A pure-C JNI frame can't unwind that, so it would propagate past `Java_..._fullTranscribe` and abort the process. Mitigation: `app/src/main/cpp/whisper_safe.cpp` wraps `whisper_full` and `whisper_init_from_file_with_params` in C++ try/catch and returns stable rcs — `-2` for recoverable Vulkan errors (DeviceLost / OutOfDeviceMem / OutOfHostMem / OutOfDate), `-3` for other `std::exception`s. `WhisperJniEngine` treats rc=-2 as "free the GPU context, init a fresh CPU one, rerun the same samples"; the user's transcription completes on CPU instead of crashing. Don't strip these wrappers — they are the only thing keeping driver-side runtime errors from killing the app.
